...
class _AchievementScreenState extends ConsumerState<AchievementScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          // Í≥µÌÜµ ?±ÏÜå ?åÎßà Î∞∞Í≤Ω (?Ä?Ä?òÍ≤å ?¨Í≥º)
          Positioned.fill(
            child: Opacity(
              opacity: 0.15,
              child: Image.asset(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '?¨ÏÑ±: ${state.completed.length}/${allAchievements.length} (${(state.completionRate * 100).toStringAsFixed(0)}%)',
                  style: TextStyle(
                    ...
                    ? () {
                        ...
                        if (success) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text('?íé ${achievement.rewardGems} Î≥¥ÏÑù ?çÎìù!'),
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                _rankStat(context, '?óº', '${state.personalBestTower}', '??ÏµúÍ≥†Ï∏?),
                Container(
                  width: 1,
                  height: Responsive.spacing(context, 40),
                  color: Colors.white12,
                ),
                _rankStat(context, '?ìÖ', '${state.personalBestDaily}', '?ÑÏ†Ñ ÏµúÍ≥†?®Ïù¥Î∏?),
              ],
            ),
          ),
          ...
          // Î¨¥Ìïú??????Çπ
          _sectionTitle(context, '?óº Î¨¥Ìïú????Í∏∞Î°ù'),
          if (state.towerRankings.isEmpty)
            _emptyRanking(context, '?ÑÏßÅ Í∏∞Î°ù???ÜÏäµ?àÎã§. ?ëÏóê ?ÑÏ†Ñ?¥Î≥¥?∏Ïöî!')
          else
          ...
          // ?ºÏùº ?ÑÏ†Ñ ??Çπ
          _sectionTitle(context, '?ìÖ ?ºÏùº ?ÑÏ†Ñ Í∏∞Î°ù'),
          if (state.dailyRankings.isEmpty)
            _emptyRanking(context, '?ÑÏßÅ Í∏∞Î°ù???ÜÏäµ?àÎã§. ?ºÏùº ?ÑÏ†Ñ???úÏûë?òÏÑ∏??')
          else
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    isHidden ? '?àÎì† ?ÖÏ†Å' : achievement.name,
                    style: TextStyle(
