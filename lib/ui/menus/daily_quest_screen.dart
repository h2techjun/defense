// ?¥Ïõê??Î¨?- ?ºÏùº ÎØ∏ÏÖò UI
// ?ÑÎ¶¨ÎØ∏ÏóÑ ?îÏûê?? Ï∂úÏÑù Ï∫òÎ¶∞??+ ÎØ∏ÏÖò Ïπ¥Îìú + ?¨ÌÅ¥Î¶¨Ïñ¥ Î≥¥Î¨º?ÅÏûê
              ...
              children: [
                ...
                Expanded(
                  child: ListView(
                    ...
                    children: [
                      // ?Ä?Ä ?∞ÏÜç Ï∂úÏÑù Ï∫òÎ¶∞???Ä?Ä
                      _buildStreakCalendar(context, ref, questState, s),
                      SizedBox(height: 20 * s),

                      // ?Ä?Ä ?ºÏùº ÎØ∏ÏÖò Ïπ¥Îìú ?Ä?Ä
                      _buildSectionTitle('?ìã ?§Îäò??ÎØ∏ÏÖò', s),
                      SizedBox(height: 8 * s),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '?ÑÎ£å: $completed / $total | ?î• ?∞ÏÜç ${state.loginStreak}?ºÏ∞®',
                  style: TextStyle(
                ...
                Row(
                  children: [
                    ...
                    SizedBox(width: 4 * s),
                    Text(
                      'Ï¥àÍ∏∞?îÍπåÏßÄ ${_getRemainingTime()}',
                      style: TextStyle(
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                Text('?î•', style: TextStyle(fontSize: 16 * s)),
                SizedBox(width: 4 * s),
                Text(
                  '${state.loginStreak}??,
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        image: DecorationImage(
          image: const AssetImage('assets/images/objects/obj_sacred_tree.png'),
          fit: BoxFit.cover,
                    ...
                    ? () {
                        ...
                        if (success) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text('${reward.emoji} ${reward.displayName} ?çÎìù!'),
                    ...
                    Row(
                      children: [
                        if (quest.rewardPassXp > 0)
                          _rewardTag('?åü${quest.rewardPassXp}XP', AppColors.sinmyeongGold, s),
                        if (quest.rewardGold > 0)
                          _rewardTag('?™ô${quest.rewardGold}', AppColors.sinmyeongGold, s),
                        if (quest.rewardGems > 0)
                          _rewardTag('?íé${quest.rewardGems}', AppColors.skyBlue, s),
                      ],
                    ),
                  ],
                ),
              ),
              // ?òÎ†π Î∞?Î∞îÎ°úÍ∞ÄÍ∏??ÑÎ†à??
              if (isClaimed)
                // CLEAR ?§ÌÉ¨???∞Ï∂ú
                ...
                ElevatedButton(
                  onPressed: () {
                    ...
                    if (success) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text('${quest.type.emoji} Î≥¥ÏÉÅ ?çÎìù!'),
                          backgroundColor: Colors.green.shade700,
                        ),
                      );
                    }
                  ...
                  child: Text('?òÎ†π', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13 * s)),
                )
              else if (quest.type.routePath != null)
                // Î∞îÎ°úÍ∞ÄÍ∏?Î≤ÑÌäº Ï∂îÍ?
                OutlinedButton.icon(
                  onPressed: () {
                    widget.onBack();
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('?¥Îãπ Î©îÎâ¥ ??ùÑ ?†ÌÉù?¥Ï£º?∏Ïöî! ??'),
                  ...
                  label: Text('?¥Îèô', style: TextStyle(color: Colors.cyan, fontSize: 11 * s, fontWeight: FontWeight.bold)),
      ...
      onTap: isReady && !isClaimed
          ? () {
              ...
              if (success) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('?éÅ ?¨ÌÅ¥Î¶¨Ïñ¥ Î≥¥ÎÑà???çÎìù!'),
          ...
          children: [
            ...
            Text(
              isClaimed
                  ...
                  : isReady
                      ? '??ïò???òÎ†π?òÏÑ∏??'
                      : '?ºÏùº ÎØ∏ÏÖò 3Í∞úÎ? Î™®Îëê ?ÑÎ£å?òÏÑ∏??,
              style: TextStyle(
            ...
            Row(
              ...
              children: [
                _rewardTag('?íé$allClearBonusGems', AppColors.skyBlue, s),
                _rewardTag('?™ô$allClearBonusGold', AppColors.sinmyeongGold, s),
                _rewardTag('?åü${allClearBonusPassXp}XP', AppColors.sinmyeongGold, s),
              ],
            ),
          ],
        ),
